(function() {
  ...
  define(['backbone', 'mustache', 'views/partials/attachment', 'views/gallery', 'collections/attachments', 'text!templates/layouts/day_view_calendar.html'], function(Backbone, M, AttachmentView, GalleryView, Attachments, CalendarTemplate) {
    var DayView;
    return DayView = (function(_super) {
      ...
      DayView.prototype.setHeaderStyle = function() {
        if (this.nextDateUri) {
          this.$('#nextDateBtn').removeClass('disabled');
        } else {
          this.$('#nextDateBtn').addClass('disabled');
        }
        if (this.previousDateUri) {
          return this.$('#previousDateBtn').removeClass('disabled');
        } else {
          return this.$('#previousDateBtn').addClass('disabled');
        }
      };
