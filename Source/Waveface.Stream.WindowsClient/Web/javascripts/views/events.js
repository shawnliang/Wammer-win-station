(function() {
  ...
  define(['underscore', 'views/layouts/day_view', 'mustache', 'mousetrap', 'views/partials/event_sum', 'views/partials/event_detail', 'collections/events', 'models/event', 'text!templates/events.html', 'localstorage', 'com/subscriber', 'scrollTo'], function(_, DayView, M, Mousetrap, EventSumView, EventDetailView, Events, EventModel, Template, Storage, Subscriber) {
    ...
    EventsView = (function(_super) {
      ...
      EventsView.prototype.render = function(date, id) {
        ...
        if (id != null) {
          ...
          if (event != null) {
            ...
            date = this.currentEvent.get("dateUri");
          }
        }
        this.setDates(date);
        data = {
      ...
      EventsView.prototype.addOne = function(event, index, prepend) {
        ...
        if (index === 0 && !this.currentEvent) {
          return this.eventSelect(event);
        }
      };

      EventsView.prototype.add1 = function(model) {
        var currentDate, currentId, state;
        state = new Storage('nc_view_state:events');
        state.data.date = currentDate = state.data.date || model.get("dateUri");
        state.data.id = currentId = state.data.id || model.id;
        state.save();
        if (model.get("dateUri") === currentDate) {
          this.eventView[model.id] = new EventSumView({
            model: model
          });
          this.$('#events-left').append(this.eventView[model.id].render().el);
        }
        if (model.id === currentId) {
          return this.eventSelect(model);
        }
      };
        ...
        if (ns === "new:event") {
          _.each(posts, function(post) {
            ...
            if (event.get("dateUri") === _this.date) {
              ...
              return _this.render(_this.currentEvent.get("dateUri"), _this.currentEvent.id);
            }
          });
        }
        return true;
      ...
      EventsView.prototype.renderCurrentEvent = function(event) {
        ...
        this.$('#events-left').scrollTo(this.eventView[this.currentEvent.id].$el, 600, {
          offset: {
            top: -9
          }
        });
        ...
        viewState.save();
        return Router.navigate("events/" + (this.currentEvent.get("dateUri")) + "/" + this.currentEvent.id);
      };
      ...
      EventsView.prototype.setKey = function() {
        var _this = this;
        EventsView.__super__.setKey.apply(this, arguments);
