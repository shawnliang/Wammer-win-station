	...
	class Program
	{
		static void Main(string[] args)
		{
			try
			{
				...
				int limit = (arguments["--limit"] != null) ? int.Parse(arguments["--limit"]) : 1;
				...
				List<GetLatestThread> workers = new List<GetLatestThread>();
				for (int i = 0; i < threads; i++)
				{
					GetLatestThread thread = new GetLatestThread(user, repeat, limit, start_event, station);
					workers.Add(thread);
					thread.Start();
				}
		...
		static void PrintUsage()
		{
			...
			Console.WriteLine("--threads n              getLatest thread numbers. default = 1");
			...
			Console.WriteLine("--limit n                limit parameter of getLatest. default = 1");
