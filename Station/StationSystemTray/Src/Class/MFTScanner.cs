...
public class MFTScanner
{
	...
	public delegate void Progress_Delegate(string sMessage);
	public delegate void IsMatch_Delegate(string sFileName, FileAttribute eAttributes, ref bool bIsMatch);
	public delegate void FileFound_Delegate(string sFileName, long lSize);
	...
	public void FindAllFiles(string szDriveLetter, FileFound_Delegate fFileFound, Progress_Delegate fProgress, IsMatch_Delegate fMatch)
	{
		USN_RECORD usnRecord = default(USN_RECORD);
		MFT_ENUM_DATA mft = default(MFT_ENUM_DATA);
		int dwRetBytes = 0;
		int cb = 0;
		...
		// progress 
		if ((fProgress != null))
			fProgress.Invoke("Building file list.");
		...
		do {
			if (DeviceIoControl(m_hCJ, FSCTL_ENUM_USN_DATA, ref mft, Marshal.SizeOf(mft), m_Buffer, m_BufferSize, ref dwRetBytes, IntPtr.Zero)) {
				...
				while ((dwRetBytes > 8)) {
					...
					bool bIsMatch = true;
					if ((fMatch != null))
						fMatch.Invoke(FileName, usnRecord.FileAttributes, ref bIsMatch);
		...
		if ((fProgress != null))
			fProgress.Invoke("Parsing file names.");
		...
		foreach (FSNode oFSNode in dicFRNLookup.Values.Where(o => o.IsFile)) {
			...
			if ((fFileFound != null))
				fFileFound.Invoke(sFullPath, 0);
		}

		//// cleanup
		Cleanup();
		if ((fProgress != null))
			fProgress.Invoke("Complete.");
	}
}
}
