...
namespace Waveface
{
	public class GCONST
	{
		...
		public string UserResourceFolder;
		public string UserThumbnailFolder;
		...
		public GCONST(RunTime runTime)
		{
			...
			InitAppDataPath();
			InitTempDir();
			InitUserResourceFolder();
			InitUserThumbnailFolder();
		...
		private void InitUserResourceFolder()
		{
			var user = _runTime.Login.user;

			UserResourceFolder = ((string)Microsoft.Win32.Registry.GetValue(WammerKeyPath, "ResourceFolder", null)) ??
							Path.Combine(AssemblyPath, "resource");

			UserResourceFolder = Path.Combine(UserResourceFolder, "user_" + user.user_id);
		}

		private void InitUserThumbnailFolder()
		{
			UserThumbnailFolder = Path.Combine(Path.Combine(AssemblyPath, "cache"), _runTime.Login.user.user_id);
		}
