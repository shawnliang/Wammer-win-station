	...
	public partial class Main : Form
	{
		...
		private void ShowPostInTimeline()
		{
			...
			else
			{
				...
				if (posts.Any(x => "image".Equals(x.type) && x.style != null && x.style.Contains("url_history")))
					this.postsArea.EnableWebHistoryFilter();
				else
					this.postsArea.DisableWebHistoryFilter();


				var driverCollection = Waveface.Libs.StationDB.StationDB.GetCollection("drivers");
				BsonDocument _userInfo = driverCollection.FindOne(Query.EQ("_id", RT.Login.user.user_id));
				...
				{
					driverCollection.Update(Query.EQ("_id", RT.Login.user.user_id), MongoDB.Driver.Builders.Update.Set("isDataImportQueried", true));
